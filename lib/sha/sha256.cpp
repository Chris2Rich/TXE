...
std::vector<unsigned char> sha256(std::vector<unsigned char> inp) {
  ...
  uint32_t hashes[] {h0, h1, h2, h3, h4, h5, h6, h7};

  for (uint32_t hash : hashes) {
      digest.push_back((hash >> 24) & 0xFF);
      digest.push_back((hash >> 16) & 0xFF);
      digest.push_back((hash >> 8) & 0xFF);
      digest.push_back(hash & 0xFF);
  }
